use libc::{mmap, munmap, shmget, shmat, shmdt, shmctl, c_void};
use libc;
use std::ptr;
use std::slice;
use std::io::Write;
...
impl RingBuffer {
    unsafe fn allocate(pages: usize) -> Option<RingBuffer> {
        ...
        let shm_id = shmget(0,
                            bytes,
                            libc::IPC_CREAT | 0o700);
        if shm_id < 0 {
            panic!("shmget failed")
        };

        // Compute the bottom half of the memory area.
        let bottom = (address as *mut u8).offset(bytes as isize)
                        as *mut libc::c_void;
    ...
    pub fn write_at_tail(&mut self, data: &[u8]) -> usize {
        ...
        self.tail = self.tail.wrapping_add(write);
