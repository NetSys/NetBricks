...
#[test]
fn address_inline() {
    let pfx = Ipv4Prefix::new(Ipv4Addr::from_str("192.168.0.0").unwrap(), 16);
    ...
    let pfx = Ipv4Prefix::new(Ipv4Addr::from_str("192.168.1.2").unwrap(), 32);
    ...
    let pfx = Ipv4Prefix::new(Ipv4Addr::from_str("0.0.0.0").unwrap(), 0);
    assert!(pfx.in_range(u32::from(Ipv4Addr::from_str("192.168.1.2").unwrap()),));
