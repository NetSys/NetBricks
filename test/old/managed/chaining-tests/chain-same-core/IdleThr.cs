...
namespace E2D2 {
	public sealed class IdleChainingTest {
		...
		public static void Main(string[] args) {
			...
			if (args.Length - options.endIdx < 3) {
				Console.WriteLine("Must specify idle, threads and length");
				Environment.Exit(1);
			}
			UInt64 cycles = Convert.ToUInt64(args[options.endIdx]);
			int threads = Convert.ToInt32(args[options.endIdx + 1]);
			int length = Convert.ToInt32(args[options.endIdx + 2]);
			Console.WriteLine("Chain length is {0}", length);
