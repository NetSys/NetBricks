...
fn test(ports: Vec<CacheAligned<VirtualQueue>>, sched: &mut Scheduler, delay_arg: u64) {
    for _ in &ports {
        println!("Receiving virtual port w/ delay {}", delay_arg);
    }
...
fn main() {
    ...
    let configuration = if matches.opt_present("c") {
        ...
        cores.dedup();
        NetbricksConfiguration { cores: cores, ..configuration }
    } else {
        configuration
    };
