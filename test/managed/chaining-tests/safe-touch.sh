...
bw=40000000000
	...
	do
		for iter in {0..2}
		do
			out="logs/isolate-$bw-$chain-$size-$iter.txt"
			...
			sudo LD_PRELOAD=libintel_dpdk.so /home/apanda/mono-bin/bin/mono --gc=sgen --llvm bin/BaselineSafeT.exe -r 4 -t 4 -- $chain $chain&
		...
		for iter in {0..2}
		do
			out="logs/noisolate-$bw-$chain-$size-$iter.txt"
			echo "Testing without isolation ${bw} iter ${iter}"
			sudo SCENARIO=s2v2s $SOFTNIC_DIR/softnic -c 4,5,6,7 -- -l 1 -d $duration -r 4 -t 4 -b $bw -s $size -w $chain | tee "$out" &
			SOFTNIC_PID=$!
			sudo LD_PRELOAD=libintel_dpdk.so /home/apanda/mono-bin/bin/mono --gc=sgen --llvm bin/BaselineT.exe -r 4 -t 4 --  $chain $chain&
			echo "Waiting for $SOFTNIC_PID"
			wait $SOFTNIC_PID
			sudo pkill mono
		done
		for iter in {0..2}
		do
			out="logs/isolate-u-$chain-$size-$iter.txt"
		...
		for iter in {0..2}
		do
			out="logs/noisolate-u-$chain-$size-$iter.txt"
		...
		do
			out="logs/ntouch-$bw-$chain-$size-$iter.txt"
			echo "Testing with isolation ${bw} iter ${iter}"
			...
			for ((component=0; component<$chain; component++)); do
				...
				sudo unbuffer $HOME/softnic/libsn/iso_test -i $vport -o $vport -c $core -t &
				sleep 1
			done
			echo "Waiting for $SOFTNIC_PID"
			wait $SOFTNIC_PID
		...
		for ((iter=0;iter<3;iter++))
		do
			out="logs/ntouch-u-$chain-$size-$iter.txt"
			echo "Testing with isolation ${bw} iter ${iter}"
