...
do
	...
	do
		...
		SOFTNIC_PID=$!
		sudo LD_PRELOAD=libintel_dpdk.so /home/apanda/mono-bin/bin/mono --gc=sgen --llvm bin/BaselineSafe.exe -r 4 -t 4 -- $chain&
	...
	do
		...
		sudo SCENARIO=v2s2v $SOFTNIC_DIR/softnic -c 4,5,6,7 -- -l 1 -d $duration -r 4 -t 4 -b $bw | tee "$out" &
		SOFTNIC_PID=$!
		sudo LD_PRELOAD=libintel_dpdk.so /home/apanda/mono-bin/bin/mono --gc=sgen --llvm bin/Baseline.exe -r 4 -t 4 -- $chain&
		echo "Waiting for $SOFTNIC_PID"
		wait $SOFTNIC_PID
		sudo pkill mono
	done
