...
#bw=700000000
	...
	do
		...
		do
			...
			do
				out="logs/csidle-$bw-$chain-$size-$idle-$iter.txt"
				...
				for ((pipeline=0; pipeline<$chain; pipeline++)); do
					...
					for ((component=0; component<$chain; component++)); do
						...
						sudo unbuffer $HOME/softnic/libsn/iso_test -i $vport -o $vport -c $pcore -w $idle -v $flc -e | tee "log-$pipeline-$component"&
						(( flc += 1 ))
						sleep 1
					done
				done
